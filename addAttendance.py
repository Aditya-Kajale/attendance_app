...
def addattendance_daily(data,present,roll,torp):
    ...
    if year == 'BTECH':
        ...
        if table_exist:
            ...
            if torp == "Theory":
                for i in roll:
                    sql = "UPDATE `{}` SET `{}`=0 WHERE roll=%s ".format(tableName,col[tt[time]])
                    val = (i,)
                    dcse_cur.execute(sql,val)
                    dcse.commit()
                
                for i in present:
                    sql = "UPDATE `{}` SET `{}`=1 WHERE roll=%s ".format(tableName,col[tt[time]])
                    val = (i,)
                    dcse_cur.execute(sql,val)
                    dcse.commit()
                ...
                for i in roll:
                    ...
                    sql = "UPDATE `{}` SET `{}`=0 WHERE roll=%s ".format(tableName,col[tt[time]+1])
                    val = (i,)
                    dcse_cur.execute(sql,val)
                    dcse.commit()

                    sql = "UPDATE `{}` SET `prsub`='{}' WHERE roll = %s".format(tableName,sname)
                    val = (i,)
                    dcse_cur.execute(sql,val)
                    dcse.commit()
                
                for i in present:
                    sql = "UPDATE `{}` SET `{}`=2 WHERE roll=%s ".format(tableName,col[tt[time]])
                    val = (i,)
                    dcse_cur.execute(sql,val)
                    dcse.commit()
                for i in present:
                    sql = "UPDATE `{}` SET `{}`=2 WHERE roll=%s ".format(tableName,col[tt[time]+1])
                    val = (i,)
                    dcse_cur.execute(sql,val)
                    dcse.commit()
            ...
            for k in range(len(total['data'])):
                # print(total['data'][k])    
                sql = "INSERT INTO `{}` (`roll`, `name`) VALUES ('{}','{}')".format(tableName, total['data'][k][0], total['data'][k][1]) 
                dcse_cur.execute(sql)
                dcse.commit()
